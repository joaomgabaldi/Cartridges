...
class RateLimiter(AbstractContextManager):
    ...
    * burst_tokens - Max number of tokens that can be consumed instantly
    ...
    def seed_history(self) -> int:
        ...
        nothing: the Steam limiter reloads up to 200 timestamps at startup, and
        a bucket created full on top of that let a restart 10 seconds after a
        big import issue another 100 requests straight away — 300 inside a
        period the service documents as allowing 200.

        Only picks still inside the period count (``len(pick_history)`` drops
        the older ones), and never more than the bucket holds. The acquires are
        non-blocking on purpose: this runs during ``__init__``, before the
        refill thread exists, so anything that blocked here would deadlock.
    ...
    def __init__(self) -> None:
        ...
        # Initialize the token bucket, then take back what a restored history
        # has already spent — the bucket has to exist before it can be drained,
        # which is why this is not done in `_init_pick_history` itself.
