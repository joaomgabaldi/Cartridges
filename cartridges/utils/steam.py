...
# and silently fail. Both English (Steam's API) and Portuguese (our stored
# output and user input) names are accepted.
_MONTH_NUMBERS = {
    # English (Steam API)
    "jan": 1, "january": 1,
    "feb": 2, "february": 2,
    ...
    "nov": 11, "november": 11,
    "dec": 12, "december": 12,
    # Portuguese (stored output + manual input)
    "janeiro": 1,
    "fev": 2, "fevereiro": 2,
    "marco": 3, "março": 3,
    "abr": 4, "abril": 4,
...
def parse_release_date(raw: str) -> tuple[Optional[int], Optional[int]]:
    ...
    what `datetime.strptime("%b")` would do. Both languages are in that table
    because both occur — Portuguese is what the API is asked for now
    ("17/set./2020") and what the app stores, English is what older records
    hold from when the request was made in it.
