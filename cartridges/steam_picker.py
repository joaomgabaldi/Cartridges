...
class SteamPicker(Adw.Dialog):
    ...
    def _search_thread(self, query: str, generation: int) -> None:
        try:
            ...
            GLib.idle_add(self._show_empty, _("Nenhum jogo encontrado"), generation)
            return
    ...
    def _show_results(
        ...
        if not self._rows:
            return self._show_empty(_("Nenhum jogo encontrado"), generation)
    ...
    def _describe(appid: str, match: TitleMatch) -> str:
        ...
        if match.confident:
            return _("appid {} · corresponde ao título").format(appid)
        return _("appid {} · parece ser outro produto").format(appid)
    ...
    def _select_thread(self, appid: str) -> None:
        try:
            ...
            GLib.idle_add(self._show_empty, _("Esse item não é um jogo"))
            return
        except (SteamError, requests.RequestException) as error:
            logging.warning("Steam picker fetch failed: %s", error)
            GLib.idle_add(self._show_empty, _("Não foi possível obter os dados"))
            return
