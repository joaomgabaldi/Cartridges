...
class CartridgesPreferences(Adw.PreferencesDialog):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._sgdb_key_changed_id = self.sgdb_key_entry_row.connect(
            "changed", sgdb_key_changed
        )

        self.sgdb_key_group.set_description(
            _(
                "É necessária uma chave da API para usar o SteamGridDB. {}Obtenha aqui{}."
            ).format(
        ...
        self._sgdb_key_sensitive_id = self.sgdb_key_entry_row.connect(
            "changed", set_sgdb_sensitive
        )
        ...
        self._process_grace_changed_id = self.process_grace_spin_row.connect(
            "notify::value",
    ...
    def reler_do_schema(self) -> None:
        ...
        não pode regravar o schema por cima do valor que acabou de ser
        restaurado nem disparar o efeito colateral de prévia das fitas
        (``mudar_brilho_padrao``/``mudar_cor_app`` acendem as fitas de LED).
        """
        self.sgdb_key_entry_row.handler_block(self._sgdb_key_changed_id)
        self.sgdb_key_entry_row.handler_block(self._sgdb_key_sensitive_id)
        try:
        ...
        self.process_grace_spin_row.handler_block(self._process_grace_changed_id)
        try:
            self.process_grace_spin_row.set_value(
                shared.schema.get_int("process-tracking-grace")
            )
        finally:
            self.process_grace_spin_row.handler_unblock(
                self._process_grace_changed_id
            )
    ...
    def block_session_monitor_options(self) -> None:
        ...
        subtitle = _("Requer um segundo monitor")
    ...
    def setup_fita_rows(self) -> None:
        ...
        Só as conexões — o preenchimento a partir do schema é
        ``reler_do_schema``, chamado uma vez no fim do ``__init__``.
        """
        ...
        # Ligado só agora, depois do `bind_switches`: o interruptor mexe no
        # próprio `active` durante a construção da tela, e abrir as
        # Preferências não pode acender fita nenhuma. Daqui em diante, quem
        # mexe no interruptor é o usuário.
    ...
    def ask_metadata_scope(self, *_args: Any) -> None:
        ...
        dialog = Adw.AlertDialog.new(
            ...
            _(
                "Buscar o que falta atualiza {} de {} jogos, consultando somente "
                "os campos ausentes. Buscar tudo preenche todos os campos, podendo "
                "levar mais tempo."
            ).format(len(incomplete), len(games)),
        )
        dialog.add_response("cancel", _("Cancelar"))
        dialog.add_response("missing", _("Buscar o que falta"))
        ...
        def on_response(_dialog: Adw.AlertDialog, response: str) -> None:
            ...
            self.add_toast(
                Adw.Toast.new(
                    _("Todos os jogos já estão com os metadados completos")
                    if not wanted
                    else _("Uma atualização de metadados já está em andamento")
                )
            )
    ...
    def remove_all_games(self, *_args: Any) -> None:
        ...
        dialog = Adw.AlertDialog.new(
            _("Remover todos os jogos?"),
            _(
                "Tem certeza que deseja remover todos os jogos da biblioteca? "
                "Esta ação é irreversível."
            ),
        )
        dialog.add_response("cancel", _("Cancelar"))
        dialog.add_response("reset", _("Remover"))
