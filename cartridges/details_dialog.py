...
class DetailsDialog(Adw.Dialog):
    ...
    def apply_preferences(self, *_args: Any) -> None:
        ...
        if self._loading_ops:
            return
        # Um jogo REMOVIDO (zerado) pode ter saído da store enquanto a tela
        # estava aberta: uma ligação de zerado por appID (`ligacao_zerado`)
        # rodando por baixo, ou uma importação que apagou a tumba. Aplicar
        # ressuscitaria um registro apagado e contaria o tempo em dobro numa
        # fusão futura. Um jogo vivo nunca é apagado da store por baixo assim
        # (só Remover/Excluir, ações da própria tela), e um jogo novo ainda
        # sem `add_game` não está na store por design — o guard é só do zerado.
        if (
            self.game is not None
            and self.game.removed
            and shared.store.get(self.game.game_id) is not self.game
        ):
            self.close()
            return
        ...
        appid_anterior = self.game.steam_appid
    ...
    def update_wallpaper_row(self) -> None:
        if not self._has_secondary_monitor:
            ...
            self.wallpaper_row.set_subtitle(_("Requer um segundo monitor"))
            return
