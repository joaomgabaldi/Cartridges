...
# What GET <INIT_URL> answers since 23/09/2026. The token is base64 of
# "<ms>::<ip>|<user-agent>|..." — hence bound to the session.
INIT_RESPONSE = {
...
class TestCredentialHandshake(unittest.TestCase):
    """The guarded search: a session-bound token."""

    def test_the_token_is_the_whole_credential(self):
        session = FakeSession()
        entries = make_helper(session).search("The Outer Worlds")

        self.assertEqual(len(entries), len(SEARCH_RESULTS))
        self.assertEqual(session.sent_headers[0]["x-auth-token"], INIT_RESPONSE["token"])
